...
const CompanyDashboard = () => {
    ...
    const getCompanyProfileData = async () => {
      try {
        ...
        if (storedProfile && storedProfile.companyProfiles && storedProfile.companyProfiles.length > 0) {
          ...
          if (companyExternalId) {
            console.log("Found company externalId from stored profile:", companyExternalId);
            setCompany(storedProfile.companyProfiles[0]);
            setCompanyId(companyExternalId);
            return; // Exit if we found the ID
          }
        }
        ...
        if (profileData && profileData.companyProfiles && profileData.companyProfiles.length > 0) {
          ...
          if (companyExternalId) {
            console.log("Found company externalId from API:", companyExternalId);
            setCompanyId(companyExternalId);
            setCompany(profileData.companyProfiles[0]);
            return;
          }
        }
  ...
  let navLinks = NAV_LINKS[UserRole.COMPANY.toLowerCase()]

  console.log('company profile s ', company)

  navLinks = useMemo(() => {
    return navLinks.map(link => ({
      ...link,
      to: link.to.replace(':companyId', companyId),
    }));
  }, [companyId]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100/50 flex">
      ...
      >
        ...
        <div className={`p-4 border-t border-blue-100 ${(isSidebarExpanded || isMobileMenuOpen) ? 'px-4' : 'px-3'}`}>
          <div
            onClick={() => navigate(`/company/dashboard/profile/${companyId}`)}
            className={`flex items-center cursor-pointer hover:bg-gray-100 ${isRouteActive('/company/dashboard/profile') ? 'bg-blue-50 text-blue-600' : ''} rounded-lg p-2 transition-colors`}
          >
